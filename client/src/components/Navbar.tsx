...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Events", href: "/events", icon: <Lightning size={20} /> },
    ...(user ? [{ name: "My Passes", href: "/profile", icon: <Ticket size={20} /> }] : []),
  ];

  return (
    <nav
      className={`fixed w-full z-50 top-0 transition-all duration-300 border-b ${scrolled ? "bg-void-black/95 backdrop-blur-md border-neon-slime/20 py-4" : "bg-transparent border-transparent py-6"}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-12">
          ...
          <Link
            to="/"
            className="flex-shrink-0 flex items-center gap-2 cursor-pointer group hover:rotate-1 transition-transform"
          >
            ...
            <span
              className="font-display font-black text-3xl tracking-tighter text-white group-hover:text-neon-slime transition-colors italic glitch"
              data-text="QRAZY"
            >
              QRAZY
            </span>
          ...
          <div className="hidden md:flex items-center gap-6">
            {user ? (
              <div className="flex items-center gap-6">
                <Link
                  to="/profile"
                  className="flex items-center gap-2 text-sm font-mono font-bold text-white hover:text-neon-slime transition-colors uppercase tracking-widest group"
                >
              ...
              <>
                <Link
                  to="/login"
                  className="text-sm font-mono font-bold text-white hover:text-hot-pink transition-colors uppercase tracking-widest border-b-2 border-transparent hover:border-hot-pink"
                >
                  Log in
                </Link>
                <Link
                  to="/clubs"
                  className="btn-primary transform hover:scale-105 transition-transform px-6 py-2"
                >
                  Get Passes
                </Link>
              </>
              ...
              {user ? (
                <>
                  <Link
                    to="/profile"
                    onClick={() => setIsOpen(false)}
                    className="w-full flex items-center justify-between py-4 px-4 bg-white/5 border border-white/10 text-white font-mono font-bold uppercase tracking-widest"
                  >
                    <span>{dbUser?.name || user.email?.split("@")[0]}</span>
                    <UserIcon weight="bold" />
                  </Link>
                ...
                <>
                  <Link
                    to="/login"
                    onClick={() => setIsOpen(false)}
                    className="w-full py-4 text-center text-white font-mono font-bold hover:text-hot-pink border-2 border-transparent hover:border-hot-pink uppercase tracking-widest"
                  >
                    Log in
                  </Link>
                  <Link
                    to="/clubs"
                    onClick={() => setIsOpen(false)}
                    className="w-full btn-primary bg-neon-slime text-void-black text-center py-4"
                  >
                    Get Passes
                  </Link>
                </>

...
export default function Events() {
  ...
  const filteredEvents = events.filter((event) => {
    if (!selectedDate || selectedDate === "weekend" || selectedDate === "custom") return true;
        ...
        <div className="flex gap-6 overflow-x-auto pb-8 scrollbar-hide snap-x">
          {highlights.length > 0 ? (
            highlights.map((event) => (
              <div
                key={`high-${event.id}`}
                className="min-w-[350px] md:min-w-[500px] h-[350px] relative overflow-hidden group snap-center border border-white/5"
              >
                <img
                  src={
                    event.club.image ||
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  ...
                  <Link
                    to={`/event/${event.id}`}
                    className="inline-flex items-center gap-4 text-neon-slime font-mono font-black text-[10px] uppercase tracking-widest group-hover:translate-x-2 transition-transform"
                  >
            ...
            <div className="w-full h-[350px] flex flex-col items-center justify-center border border-dashed border-white/5 bg-white/5">
              <span className="text-[10px] font-mono text-brand-gray/20 uppercase tracking-[0.5em]">
                No_Active_Frequencies
              </span>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        <div className="lg:w-64 shrink-0">
          <div className="sticky top-32 space-y-8">
            <div>
              <h3 className="text-[10px] font-mono font-black text-brand-gray/40 uppercase tracking-[0.3em] mb-6">
                Discovery Window
              </h3>
              <div className="space-y-2">
                <button
                  onClick={() => setSelectedDate(null)}
                  className={`w-full flex justify-between items-center p-4 border transition-all ${selectedDate === null
                      ? "bg-neon-slime/10 border-neon-slime text-neon-slime"
                      : "bg-white/5 border-white/10 text-brand-gray/60 hover:border-white/20"
                    }`}
                >
                  <span className="font-mono text-[10px] uppercase font-bold text-left">All Access</span>
                ...
                {dates.map((date) => (
                  ...
                  >
                    <span className="font-mono text-[10px] uppercase font-bold text-left">{date.label}</span>
                    <CaretRight weight="bold" size={12} />
                  </button>
                ))}
              </div>
            ...
            <div className="p-6 bg-white/5 border border-white/10">
              <h4 className="text-[10px] font-mono font-black text-neon-slime uppercase mb-2">
                VIP Portal
              </h4>
        ...
        <div className="flex-grow">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-32 gap-4">
              ...
              <span className="font-mono text-xs text-brand-gray/40 uppercase tracking-widest">
              ...
              {filteredEvents.length > 0 ? (
                filteredEvents.map((event) => (
                  ...
                  >
                    ...
                    <div className="p-6">
                      <div className="text-neon-slime font-mono text-[9px] uppercase tracking-widest mb-1">
                        {event.club.name}
                      </div>
                ...
                <div className="col-span-full py-32 text-center border border-dashed border-white/10 bg-white/5 translate-y-4">
                  <div className="text-neon-slime font-mono text-[10px] uppercase tracking-[0.5em] mb-3 opacity-60">
                    No_Matching_Frequencies
                  </div>
                  <div className="text-brand-gray/40 font-mono text-[8px] uppercase tracking-widest">
                    Try searching for another artist or adjusting the discovery window
                  </div>
                </div>
              )}
            </div>

...
export default function EventDetails() {
  ...
  if (!club) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        <h2 className="text-4xl font-display font-black text-white uppercase italic mb-4 text-glow-pink">
          Sector Redacted
        </h2>
        <p className="text-brand-gray/60 font-mono mb-8">
          This club is currently offline or doesn't exist.
        </p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Link
        to="/clubs"
        className="flex items-center gap-2 text-brand-gray/60 hover:text-neon-slime font-mono text-sm mb-8 transition-colors group"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2">
          ...
          <section className="mb-12">
            <h2 className="text-2xl font-display font-black text-white uppercase mb-4 tracking-tight">
              Transmission Details
            </h2>
            <p className="text-brand-gray/70 font-sans leading-relaxed text-lg">
              {club.description ||
                "The city's hottest underground sanctuary. Experience pure sonic chaos. Strictly 21+. Proper club attire requested."}
            </p>
          </section>

          <section>
            <h2 className="text-2xl font-display font-black text-white uppercase mb-6 tracking-tight">
              Security Protocols
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div
                  key={i}
                  className="flex items-center gap-2 p-4 bg-white/5 border border-white/10 text-brand-gray/80 font-mono text-sm"
                >
          ...
          <div className="glass-panel p-8 sticky top-32 border-hot-pink/20 shadow-pink-glow">
            ...
            {!event || !event.passTypes || event.passTypes.length === 0 ? (
              <div className="py-8 text-center border border-white/5 bg-white/5">
                <p className="text-brand-gray/40 font-mono text-xs uppercase">
                  No passes currently listed.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {event.passTypes.map((pass: PassType) => (
                  <div
                    key={pass.id}
                    className="p-4 bg-void-black border border-brand-gray/10 hover:border-neon-slime transition-colors group"
                  >
                    ...
                    <div className="flex justify-between items-center mt-4">
                      ...
                      <button
                        ...
                        className={`text-[10px] font-black uppercase px-4 py-2 border transition-all ${
                          pass.capacity > 0
